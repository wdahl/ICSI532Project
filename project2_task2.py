...
# scripts from precious tasks
from project2_task1 import get_weight_edge_list
...
# we assume that the first task has been performed, so the weighted edge list has been created
# and can be used directly

Graph = nx.DiGraph() # Directed Graph for the emails
weighted_edge_list = get_weight_edge_list()
...
def compute_1_5_egonet(graph: Graph):
    '''
    This function will compute each node n's 1.5 egonetwork and write it into a json file.
    the file should be like= {"G_u1": {"E":[],"V":[]}, "G_u2": {"E":[], "V":[]} }
    '''
    processed_data = dict()
    designated_file_path = './processed data/1_5_egonet_data.txt'
    
    edge_dict= dict()
    for u,v,weight in weighted_edge_list:
        if frozenset([u,v]) not in edge_dict:
            edge_dict[frozenset([u,v])] = int(weight)
        else:
            edge_dict[frozenset([u,v])] += int(weight)
    
    graph_copy = graph.copy().to_undirected # transform it into undirected graph
    
    for i,n in enumerate(graph_copy): # computing 1.5 ego net
        print(f"\rProcessing node {n}... {i+1/len(graph):.4%}",end="")
        
        Vs = list(set(graph_copy.neighbors(n)).union({n}))
        Es = [] # edges in the form (u, v, weight)
        
        if len(Vs) >= 2:
            for item in combinations(Vs,2): # for every pair of the nodes
                if frozenset(item) in edge_dict:
                    Es.append([item[0],item[1],edge_dict[frozenset(item)]])
        
        processed_data[n] = {"V": Vs, "E": Es}
        print(processed_data)
        breakpoint()
    print(f"Done !!!")
    
    # if not path.exists(designated_file_path): # if the file not presented, added it to the file
    with open(designated_file_path,'w') as outfile:
        json.dump(processed_data , outfile)

compute_1_5_egonet(Graph)

ego1_5 = dict()
with open('./processed data/1_5_egonet_data.txt','r') as f:
    ego1_5 = json.load(f)
...
for i in ego1_5:
    ...
    temporal_graph.add_weighted_edges_from(i["E"])
...
plt.loglog(node_counts, edge_fit, label=f'log y = {m:.6} log x  {c:.6}') # Plots the line of best fit
...
plt.loglog(node_counts, stars,'--',alpha=0.4, label='Stars')
plt.loglog(node_counts, cliques,'--',alpha=0.4, label='Cliques')
