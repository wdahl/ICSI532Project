...
def compute_diameter(graph: 'Graph'):
    '''
    this function will take in a graph, compute and print the diameter of the largest components
    '''
    ...
    largest_scc = nx.subgraph( graph, strongly_cc_nodes)
    for i,n in enumerate(largest_scc.nodes()):
        print(f"\rProcessing... {i/len(largest_scc):.4%}",end='')
        value = nx.eccentricity(largest_scc,n)
        with open('./processed data/task2diameter.txt','a') as f:
            f.write(f'{n} {value}\n')
        
    # largest_scc.add_weighted_edges_from(scc_edges)
    # print(f"compute time: {duration(s)}")
    # print(f"The diameter of this graph is: {nx.diameter(largest_scc)}")

# compute_diameter(Graph)
diameter = 0
with open('./processed data/task2diameter.txt', 'r') as f:
    for line in f.readlines():
        line.strip()
        u, diamtr = line.split(' ')
        diamtr = int(diamtr)
